...
export default function Header() {
    ...
    />
                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        {navLinks.map((link) => (
                            <a
                                key={link.name}
                                href={link.href}
                                className="text-pink-800 hover:text-pink-500 font-medium transition-colors duration-200 relative group"
                            >
                                {link.name}
                    ...
                    <button
                        ...
                        className="md:hidden p-2 rounded-lg text-pink-600 hover:bg-pink-100 transition-colors"
                    >
                        <svg
                            className="w-6 h-6"
                            fill="none"
                ...
                >
                    <nav className="flex flex-col space-y-4 py-4 px-2 glass rounded-2xl">
                        {navLinks.map((link) => (
                            <a
                                key={link.name}
                                href={link.href}
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="text-pink-800 hover:text-pink-500 font-medium py-2 px-4 rounded-lg hover:bg-pink-100 transition-all"
                            >
                                {link.name}
                            </a>
                        ))}
